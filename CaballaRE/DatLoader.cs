...
namespace CaballaRE
{
    class DatLoader
    {
            ...
            public void UpdateTable()
            {
                if (datatable != null)
                {
                    ...
                    for (int i = 0; i < datatable.Rows.Count; i++)
                    {
                        ...
                        {
                            object v = dr[j];
                            row[j] = (v == null || v == DBNull.Value) ? "" : v.ToString();
                        }
                        this.rows.Add(row);
                    }
        ...
        // Register a new column in a table's metadata (header + FieldCnt).
        // The caller is responsible for adding the DataColumn to the live DataTable.
        public bool RegisterTableColumn(int tableId, string name, string isKey, string dataType)
        {
            if (tableId < 0 || tableId >= tables.Count) return false;
            TableInfo ti = tables[tableId];
            if (ti.GetTable().Columns.Contains(name)) return false;

            ti.AddField(name, isKey ?? "0", dataType ?? "string");
            int cnt;
            if (!int.TryParse(ti.FieldCnt, out cnt)) cnt = 0;
            ti.FieldCnt = (cnt + 1).ToString();
            return true;
        }

        // Rename a table.
        public bool RenameTable(int tableId, string newName)
        {
            if (tableId < 0 || tableId >= tables.Count) return false;
            if (string.IsNullOrWhiteSpace(newName)) return false;
            tables[tableId].Name = newName;
            return true;
        }

        // Create a new empty table and append it to the table list.
        // Returns the index of the newly created table, or -1 on failure.
        public int AddTable(string name)
        {
            if (string.IsNullOrWhiteSpace(name)) return -1;

            // Generate a TableInfoID that doesn't already exist.
            int nextId = tables.Count;
            while (infotables.Contains(nextId.ToString())) nextId++;
            string tid = nextId.ToString();
            ...
            int infoCount;
            if (!int.TryParse(tableinfocount, out infoCount)) infoCount = 0;
            tableinfocount = (infoCount + 1).ToString();
